...
export default function PaymentPage() {
  ...
  const [formData, setFormData] = useState({
    paymentMethod: "bkash", // Default selection match with DB case
    senderMobile: "",
    transactionId: ""
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. Fetch Admission Details (FIXED ROUTE)
        // আগে "/admissions/my-admissions" ছিল যা 404 দিচ্ছিল
        const admRes = await api.get("/admissions/my"); 
        const found = admRes.data.find(a => a._id === id);

        if (!found) {
          try {
            // If the ID is a course ID, send the learner to the admission form instead of failing silently
            await api.get(`/courses/${id}`);
            toast.error("Please submit your admission form first.");
            router.push(`/admissions/${id}`);
          } catch (courseErr) {
            console.error("Admission lookup failed:", courseErr);
